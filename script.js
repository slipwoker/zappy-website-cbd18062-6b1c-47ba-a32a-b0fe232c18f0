document.addEventListener('DOMContentLoaded', function() {
  ...
  window.addEventListener('scroll', function() {
    ...
    if (currentScroll > 100) {
      stickyNav.style.boxShadow = '0 4px 12px rgba(0,0,0,0.15)';
    } else {
      stickyNav.style.boxShadow = '0 2px 8px rgba(0,0,0,0.08)';
    }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "קו האמת משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: 'cbd18062-6b1c-47ba-a32a-b0fe232c18f0',
                    name: data.name || '',
                    email: data.email || '',
